...
| `JS/simulate.js` | Fake sensor transport with the *same interface* as `ble.js` (`addSensor`/`disconnectSensor`/`setSampleHandler`), so the rest of the app can't tell the difference. Two selectable scenarios: a real-lift vertical pulse, and a pure-rotation spin (no translation) — the latter is the acceptance test proving `processing.js` rejects rotation instead of reporting it as fake velocity. |
